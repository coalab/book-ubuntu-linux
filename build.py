...
<div class="hd">
  ...
  <div style="display:flex;align-items:center;gap:12px">
    ...
    <button class="pdf-btn" onclick="printAll()">📄 PDF</button>
  </div>
</div>
<div class="viewer">
...
function printAll(){{
  ...
  const css=`
    *{{box-sizing:border-box;margin:0;padding:0}}
    body{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI','Noto Sans KR',sans-serif;background:#fff}}
    .page{{width:210mm;min-height:297mm;padding:18mm 20mm 14mm;page-break-after:always;overflow:hidden;display:flex;flex-direction:column;border:none}}
    .pg-hd{{border-bottom:1px solid #e5e7eb;padding-bottom:4mm;margin-bottom:4mm;font-size:8pt;color:#9ca3af}}
    .pg-bd{{flex:1;color:#1f2937;line-height:1.7;font-size:9pt}}
    .pg-bd h1{{font-size:17pt;font-weight:900;color:#111827;border-bottom:2.5pt solid #2563eb;padding-bottom:3mm;margin-bottom:5mm;line-height:1.3}}
    .pg-bd h2{{font-size:11pt;font-weight:700;color:#2563eb;margin:5mm 0 2mm}}
    .pg-bd h3{{font-size:9.5pt;font-weight:700;color:#111827;margin:4mm 0 1.5mm}}
    .pg-bd p{{margin-bottom:2.5mm}}
    .pg-bd ul,.pg-bd ol{{padding-left:6mm;margin:1mm 0 3mm}}
    .pg-bd li{{margin:1mm 0}}
    .pg-bd blockquote{{border-left:2.5pt solid #2563eb;padding:2mm 4mm;background:#eff6ff;color:#374151;font-style:italic;margin:3mm 0;border-radius:0 2pt 2pt 0}}
    .pg-bd pre{{background:#1e293b;color:#e2e8f0;padding:3mm 4mm;border-radius:2pt;font-size:7.5pt;margin:2.5mm 0;line-height:1.5;white-space:pre-wrap;word-break:break-all}}
    .pg-bd code{{background:#f0f4ff;color:#1d4ed8;padding:0.5mm 1.5mm;border-radius:2pt;font-size:8pt}}
    .pg-bd pre code{{background:none;color:inherit;padding:0;font-size:1em}}
    .pg-bd table{{width:100%;border-collapse:collapse;margin:2.5mm 0;font-size:8.5pt}}
    .pg-bd th{{background:#2563eb;color:#fff;padding:1.5mm 2.5mm;text-align:left;font-weight:600}}
    .pg-bd td{{padding:1.5mm 2.5mm;border-bottom:0.5pt solid #e5e7eb}}
    .pg-bd tr:nth-child(even) td{{background:#f9fafb}}
    .pg-bd strong{{color:#111827;font-weight:700}}
    .pg-bd hr{{border:none;border-top:0.5pt solid #e5e7eb;margin:3mm 0}}
    .pg-ft{{border-top:0.5pt solid #e5e7eb;padding-top:3mm;margin-top:3mm;text-align:center;font-size:8pt;color:#9ca3af}}
    .cov-wrap{{display:flex;flex-direction:column;height:100%;padding:0}}
    .cov-top{{border-bottom:1pt solid #e5e7eb;padding-bottom:3mm;margin-bottom:8mm;font-size:8pt;color:#9ca3af}}
    .cov-body{{flex:1;display:flex;flex-direction:column;justify-content:center}}
    .cov-ic{{font-size:32pt;margin-bottom:4mm}}
    .cov-t{{font-size:24pt;font-weight:900;color:#111827;line-height:1.25;margin-bottom:4mm}}
    .cov-sub{{font-size:11pt;color:#374151;margin-bottom:5mm}}
    .cov-line{{width:20mm;height:2.5pt;background:#2563eb;margin-bottom:6mm}}
    .cov-desc{{font-size:9pt;color:#6b7280;line-height:1.8;margin-bottom:6mm}}
    .cov-date{{font-size:8pt;color:#9ca3af}}
    .cov-ft{{font-size:8pt;color:#9ca3af;text-align:center;padding-top:3mm;border-top:0.5pt solid #e5e7eb;margin-top:auto}}
  ...
  PAGES.forEach((page,i)=>{{
    ...
    body+=`<div class="page"><div class="pg-hd">이것이 우분투 리눅스다</div><div class="pg-bd">${{md2html(page.md)}}</div><div class="pg-ft">— ${{i+1}} —</div></div>`;
  }});
  win.document.write(`<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>이것이 우분투 리눅스다</title><style>${{css}}</style></head><body>${{body}}</body></html>`);
  win.document.close();
